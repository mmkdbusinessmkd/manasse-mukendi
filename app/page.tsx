...
export default function Home() {
  ...
  return <main>
    ...
    <section className="contact section" id="contact"><div className="section-head"><p className="eyebrow">06 / CONTACT</p><h2>Parlons de<br/>votre <em>projet.</em></h2><p>Quatre informations suffisent pour commencer. Décrivez-moi votre besoin, je vous répondrai dans les meilleurs délais.</p><a className="mail" href={`mailto:${contact.email}`}>{contact.email} ↗</a></div><form onSubmit={submit}><input className="honeypot" type="text" name="_honey" tabIndex={-1} autoComplete="off"/><input type="hidden" name="_subject" value="Nouvelle demande — site Manassé Mukendi"/><input type="hidden" name="_template" value="table"/>{[["Nom", "text", "name"], ["Email", "email", "email"]].map(([p,t,n]) => <label key={p}><span>{p}</span><input name={n} required type={t} placeholder={p === "Nom" ? "Votre nom" : "votre@email.com"}/></label>)}<label className="full"><span>Service recherché</span><select name="service" required defaultValue=""><option disabled value="">Choisir un service</option>{services.map(x => <option key={x[1]}>{x[1]}</option>)}</select></label><label className="full"><span>Votre besoin</span><textarea name="message" required placeholder="Parlez-moi de votre projet, de vos objectifs et de ce que vous attendez..."></textarea></label><div className="form-end"><p>{formMessage || "Tous les champs sont nécessaires."}</p><button className="button dark">Envoyer ma demande <b>↗</b></button></div></form></section>

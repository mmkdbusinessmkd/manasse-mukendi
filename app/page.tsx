...
const services = [
  ["01", "Social Media Management", "Construire une présence régulière, cohérente et adaptée à votre audience."],
  ["02", "Stratégie digitale", "Donner une direction claire à votre communication."],
  ["03", "Communication de marque", "Travailler la manière dont votre entreprise se présente, parle et est perçue."],
  ["04", "Création de contenu", "Transformer vos idées, vos offres et vos actualités en contenus qui parlent à votre audience."],
...
const contactFields = [
  ...
  { label: "Téléphone / WhatsApp", type: "tel", name: "phone", placeholder: "Votre numéro avec indicatif pays", required: true },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      ...
      if (event.key === "Tab") {
        const controls = document.querySelectorAll<HTMLElement>(".project-modal button, .project-modal a[href]");
  ...
  return <main>
    ...
    <section className="cta"><p className="eyebrow light">PROCHAINE ÉTAPE</p><h2>Votre communication<br/>peut faire <em>mieux.</em></h2><p>Parlons de votre image, de votre communication et de ce que vous souhaitez construire.</p><div className="actions"><a className="button white" href="#contact">Parler de mon projet <BrandGlyph/></a><a className="button button-ghost" href={contact.whatsapp} target="_blank" rel="noreferrer">Échanger sur WhatsApp <BrandGlyph/></a></div></section>

    <section className="contact section" id="contact"><div className="section-head"><p className="eyebrow">04 / CONTACT</p><h2>Votre projet commence<br/>par un <em>échange.</em></h2><p>Décrivez brièvement votre besoin. Je vous recontacte personnellement pour définir la meilleure prochaine étape.</p><a className="mail" href={`mailto:${contact.email}`}>{contact.email} <span className="link-dot" aria-hidden="true"></span></a><a className="whatsapp-link contact-whatsapp" href={contact.whatsapp} target="_blank" rel="noreferrer">Ou écrivez-moi sur WhatsApp <span className="link-dot" aria-hidden="true"></span></a></div><form onSubmit={submit} aria-busy={isSubmitting}><input className="honeypot" type="text" name="_honey" tabIndex={-1} autoComplete="off"/><input type="hidden" name="_subject" value="Nouvelle demande — site Manassé Mukendi"/><input type="hidden" name="_template" value="table"/>{contactFields.map((field) => <label key={field.label}><span>{field.label}</span><input name={field.name} required={field.required} type={field.type} autoComplete={field.name === "phone" ? "tel" : field.name} placeholder={field.placeholder}/></label>)}<label><span>Service recherché</span><select name="service" required defaultValue=""><option disabled value="">Choisir un service</option><option>Je souhaite être conseillé</option>{services.map(x => <option key={x[1]}>{x[1]}</option>)}</select></label><label className="full"><span>Votre besoin</span><textarea name="message" required placeholder="Votre objectif et le résultat que vous souhaitez obtenir…"></textarea></label><div className="form-end"><p role="status" aria-live="polite">{formMessage || "Vos informations servent uniquement à vous recontacter au sujet de votre demande."}</p><button className="button dark" disabled={isSubmitting}>{isSubmitting ? "Envoi en cours…" : "Envoyer ma demande"} <BrandGlyph/></button></div></form></section>

...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="accueil"><div className="hero-copy"><p className="eyebrow light">MANASSÉ MUKENDI <span>/</span> CONSULTANT &amp; SOCIAL MEDIA MANAGER</p><h1><span className="headline-lead">La communication</span> qui <em>déplace</em> les marques.</h1><p className="intro">J’aide les marques à mieux structurer leur image, leur communication et leur présence digitale.</p><div className="actions"><a className="button white" href="#contact">Parler de mon projet <BrandGlyph/></a><a className="button button-ghost" href="#services">Découvrir mes services <span className="button-dot" aria-hidden="true"></span></a></div></div><div className="orbit" aria-hidden="true"><div className="orbit-ring"></div><div className="orbit-core"><span>digital</span><strong>impact</strong></div><span className="tag t1">STRATÉGIE</span><span className="tag t2">SOCIAL</span><span className="tag t3">CONTENU</span><span className="dot d1"></span><span className="dot d2"></span></div><div className="hero-foot"><span>Marketing · Communication · Social Media</span><span>PROJETS LOCAUX &amp; INTERNATIONAUX <b>●</b></span></div></section>

    <section className="section services" id="services"><div className="section-head"><p className="eyebrow">01 / SERVICES</p><h2>Ce que je peux faire<br/>pour votre <em>marque.</em></h2><p className="section-intro">Des accompagnements clairs, adaptés à votre situation et orientés vers un résultat concret.</p></div><div className="service-grid">{services.map(([n,t,d]) => <article className="service" key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p><a className="service-cta" href="#contact">Parler de ce besoin <span aria-hidden="true">↗</span></a></article>)}</div><div className="service-closing"><p>Vous hésitez sur le bon accompagnement ?</p><a href="#contact">Discutons de votre communication <span className="link-dot" aria-hidden="true"></span></a></div></section>

    <section className="section why"><div className="why-head"><p className="eyebrow">02 / POURQUOI TRAVAILLER AVEC MOI ?</p><h2>Comprendre avant<br/>de <em>communiquer.</em></h2></div><div className="why-grid">{[["01","Je comprends avant de publier.","Chaque action part de votre réalité, de votre audience et de vos objectifs."],["02","Je pense stratégie avant exécution.","Les contenus et les canaux suivent une direction claire, pas une simple tendance."],["03","Je relie image, communication et réputation.","Votre marque reste cohérente à chaque point de contact."]].map(([n,t,d]) => <article key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p></article>)}</div><a className="button dark why-cta" href="#contact">Discuter de votre projet <BrandGlyph/></a></section>

    <section className="work" id="realisations"><div className="work-head"><div><p className="eyebrow light">03 / RÉALISATIONS</p><h2>Des preuves,<br/>pas seulement des <em>visuels.</em></h2></div><p>Une sélection de projets représentatifs. D’autres réalisations peuvent être présentées selon votre secteur.</p></div><div className="work-proof"><span><strong>29 992</strong> personnes touchées</span><span><strong>6+</strong> secteurs accompagnés</span><span><strong>Partout</strong> projets locaux et internationaux</span></div><div className="project-grid compact-project-grid">{visibleProjects.map((p,i) => <article className="project case-study" key={p.name}><div className="project-art"><img src={p.image} alt={`${p.name} — ${p.client}`} loading="lazy" decoding="async"/><div className="project-topline"><span>{p.type}</span><BrandGlyph className="card-glyph light"/></div><b>{String(i+1).padStart(2,"0")}</b></div><div className="project-info"><p className="case-client">CLIENT <strong>{p.client}</strong> <span>/ {p.sector}</span></p><h3>{p.name}</h3><div className="case-details"><div className="case-detail"><small>CONTEXTE</small><p>{p.context}</p></div><div className="case-detail"><small>INTERVENTION</small><p>{p.intervention}</p></div><div className="case-result"><small>RÉSULTAT</small><strong>{p.result}</strong></div></div><button className="project-view-button" onClick={() => setSelectedProject(p)}>Voir le projet <span aria-hidden="true">+</span></button></div></article>)}</div><div className="work-actions">{projects.length > 4 && <button className="button project-toggle" onClick={() => setShowAllProjects(!showAllProjects)}>{showAllProjects ? "Réduire la sélection" : `Voir ${projects.length - 4} autres projets`} <span aria-hidden="true">{showAllProjects ? "−" : "+"}</span></button>}<a className="button white" href="#contact">Parler d’un projet similaire <BrandGlyph/></a></div></section>

    <section className="about about-short" id="a-propos"><div className="portrait"><div className="portrait-photo" role="img" aria-label="Portrait de Manassé Mukendi, consultant en marketing et communication digitale"></div><div className="portrait-note">MANASSÉ<br/>MUKENDI <BrandGlyph className="outline"/></div></div><div className="about-copy"><p className="eyebrow">04 / À PROPOS</p><h2>Je ne commence jamais<br/>par <em>publier.</em></h2><div className="bio editorial-bio"><p className="about-lead">Je commence par comprendre.</p><p>Votre marque, vos objectifs, votre audience et la perception que vous souhaitez construire.</p><p>C’est ce qui me permet de proposer une communication cohérente, utile et pensée pour durer.</p></div><a className="button dark about-cta" href="#contact">Parler de votre projet <BrandGlyph/></a></div></section>

    <section className="trust-compact" aria-labelledby="trust-title"><div className="trust-copy"><p className="eyebrow">CONFIANCE &amp; EXPERTISE</p><h2 id="trust-title">Des compétences solides.<br/><em>Des collaborations réelles.</em></h2><div className="certification-chips">{certifications.map((certificate) => <span key={certificate}>{certificate}</span>)}</div></div><div className="partner-grid" aria-label="Marques et organisations accompagnées">{partnerLogos.map(([name, logo]) => <div className="partner-card" key={name}><img src={logo} alt={name} loading="lazy" decoding="async"/></div>)}</div></section>

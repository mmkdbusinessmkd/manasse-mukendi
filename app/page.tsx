...
const services = [
  ["01", "Conseil en marketing et communication digitale", "Faire émerger les vrais enjeux et transformer vos objectifs en priorités de communication claires."],
  ["02", "Stratégie digitale", "Construire un positionnement distinctif, des messages cohérents et une feuille de route exploitable."],
  ["03", "Stratégie Social Media", "Installer une présence éditoriale régulière qui attire l’attention et renforce la confiance."],
  ["04", "Création de contenu", "Créer des contenus qui rendent vos offres plus simples à comprendre, à retenir et à choisir."],
  ["05", "Pilotage des réseaux sociaux", "Piloter vos prises de parole et votre communauté sans perdre la cohérence de marque."],
  ["06", "Publicité digitale", "Gagner en visibilité auprès des bonnes audiences avec des campagnes suivies et un budget maîtrisé."],
];
...
const projects = [
  ...
  { name: "Une nouvelle semaine commence", client: "ZOOM TECH", type: "Social Media", sector: "Technologie", context: "Relier les solutions technologiques aux besoins concrets des professionnels.", intervention: "Conception éditoriale et visuelle d’un contenu de début de semaine, pensé pour rendre l’offre plus accessible et favoriser l’interaction.", result: "Un contenu clair et identifiable qui traduit la promesse de Zoom Tech en bénéfices simples et concrets.", image: "/images/projects/zoom-tech-semaine.webp" },
...
const steps = [["01", "Comprendre", "Votre activité, vos objectifs, votre audience et vos priorités."], ["02", "Structurer", "Le positionnement, les messages et la feuille de route."], ["03", "Déployer", "Les contenus, campagnes et prises de parole utiles."], ["04", "Mesurer", "Les résultats, les apprentissages et les prochaines actions."]];

const proofPoints = [
  ["29 992", "personnes touchées sur une campagne documentée"],
  ["Sélection", "de projets représentatifs"],
  ["6", "secteurs déjà accompagnés"],
  ["Partout", "projets locaux et internationaux"],
];
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero" id="accueil"><div className="hero-copy"><p className="eyebrow light">MANASSÉ MUKENDI <span>/</span> MOBILITÉ INTERNATIONALE</p><p className="positioning">Consultant en marketing &amp; communication digitale <span>/</span> Social Media Manager</p><h1><span className="headline-lead">La communication</span> qui <em>déplace</em> les marques.</h1><p className="intro">J’aide les entreprises à mieux structurer leur image, leur communication et leur présence digitale pour être mieux comprises, mieux perçues et mieux choisies.</p><div className="actions"><a className="button white" href="#services">Découvrir mes services <BrandGlyph/></a><a className="button button-ghost" href="#contact">Parler de votre projet <span className="button-dot" aria-hidden="true"></span></a></div></div><div className="orbit" aria-hidden="true"><div className="orbit-ring"></div><div className="orbit-core"><span>digital</span><strong>impact</strong></div><span className="tag t1">STRATÉGIE</span><span className="tag t2">SOCIAL</span><span className="tag t3">CONTENU</span><span className="dot d1"></span><span className="dot d2"></span></div><div className="hero-foot"><span>Marketing · Communication · Social Media</span><span>RDC · AFRIQUE · INTERNATIONAL <b>●</b></span></div></section>

    <section className="proof-bar" aria-label="Repères et résultats">{proofPoints.map(([value, label]) => <div className="proof-point" key={label}><strong>{value}</strong><span>{label}</span></div>)}</section>

    <section className="section services" id="services"><div className="section-head"><p className="eyebrow">01 / ACCOMPAGNEMENT</p><h2>Une communication plus claire.<br/>Une marque plus <em>forte.</em></h2><p className="section-intro">Du diagnostic au déploiement, je relie la stratégie, les contenus et les canaux pour construire une présence qui sert réellement vos objectifs.</p></div><div className="service-grid">{services.map(([n,t,d]) => <article className="service" key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p><BrandGlyph className="card-glyph"/></article>)}</div><div className="service-closing"><p>Vous ne savez pas encore quel accompagnement choisir ?</p><a href="#contact">Commençons par clarifier votre besoin <span className="link-dot" aria-hidden="true"></span></a></div></section>

    <section className="about" id="a-propos"><div className="portrait"><div className="portrait-photo" role="img" aria-label="Portrait créatif de Manassé Mukendi, consultant en marketing et communication digitale"></div><div className="portrait-note">MANASSÉ<br/>MUKENDI <BrandGlyph className="outline"/></div></div><div className="about-copy"><p className="eyebrow">02 / À PROPOS</p><h2>Je ne commence jamais<br/>par <em>publier.</em></h2><div className="bio editorial-bio"><p className="about-lead">Je commence par comprendre.</p><p className="about-role">Mon rôle est de transformer vos enjeux de communication en une direction claire, puis en actions cohérentes.</p><p>Votre marque, vos objectifs, votre audience, votre positionnement et la perception que vous souhaitez construire guident chaque recommandation.</p><p>Mobile et disponible, j’interviens comme consultant en marketing et communication digitale et Social Media Manager auprès d’entreprises en RDC, en Afrique et à l’international, sur place ou à distance.</p></div></div></section>

    <section className="credentials credentials-compact" id="certifications"><div className="credential-intro"><p className="eyebrow">CERTIFICATIONS / COMPÉTENCES</p><h2>La pratique d’abord.<br/><em>Des repères solides.</em></h2><p>Une expertise consolidée en Community Management, Social Media, publicité digitale et stratégie de contenu.</p></div><div className="credential-list compact-list">{certifications.map((certificate, index) => <div className="credential-item" key={certificate}><span className="credential-number">0{index + 1}</span><strong>{certificate}</strong><span className="credential-check" aria-hidden="true">✓</span></div>)}</div></section>

    <section className="section method" id="methode"><div className="section-head"><p className="eyebrow">03 / PROCESSUS</p><h2>Une méthode claire.<br/>Des décisions aux<br/><em>résultats.</em></h2><p className="section-intro">Chaque mission suit un cadre simple pour éviter les actions dispersées et garder le cap sur vos priorités.</p></div><div className="timeline">{steps.map(([n,t,d]) => <article key={n}><span>{n}</span><div><h3>{t}</h3><p>{d}</p></div><b className="timeline-progress" aria-hidden="true"><span></span></b></article>)}</div></section>
    ...
    <section className="reasons"><p className="eyebrow">05 / APPROCHE</p><h2>Ce qui guide chaque<br/><em>collaboration.</em></h2><div>{[["La clarté avant l’action", "Comprendre le vrai besoin avant de produire ou de publier."], ["Une stratégie applicable", "Des recommandations réalistes, adaptées à vos moyens et à votre rythme."], ["Une marque cohérente", "Des messages et des contenus reconnaissables à chaque point de contact."], ["Des résultats observables", "Suivre ce qui progresse pour décider des prochaines actions avec méthode."]].map(([t,d],i) => <article key={t}><span>0{i+1}</span><h3>{t}</h3><p>{d}</p></article>)}</div></section>
